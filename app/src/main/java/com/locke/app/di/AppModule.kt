...
import com.locke.app.data.local.AppDatabase
...
private val Context.dataStore: DataStore<Preferences> by preferencesDataStore(name = "locke_prefs")
...
object AppModule {
    ...
    /**
     * Every version this database has ever shipped (1 through 12, current) has a real
     ...
     *
     * So: bumping `AppDatabase.version` past 12 without adding a matching
     * `MIGRATION_12_13` (etc.) here is a bug, not a style choice -- add the migration
     * (mirroring the pattern in Migrations.kt) in the same change that bumps the
     * version, the same way every step 1->12 was done.
     */
    @Provides
    @Singleton
    ...
    @Provides
    fun provideAnalyticsEventDao(db: AppDatabase): AnalyticsEventDao = db.analyticsEventDao()
