...
import com.locke.app.data.repository.ActiveDomainBlock
import com.locke.app.data.repository.BedtimeRepository
import com.locke.app.data.repository.BlockAttemptRepository
...
import com.locke.app.data.repository.LockoutRepository
import com.locke.app.data.repository.LootboxRepository
import com.locke.app.data.repository.PenaltyRepository
import com.locke.app.data.repository.PreferencesRepository
import com.locke.app.data.repository.UrlBlockRepository
...
import com.locke.app.ui.block.BlockOverlayActivity
import com.locke.app.util.BrowserUrlExtractor
...
class AppBlockAccessibilityService : AccessibilityService() {
    ...
    @Inject lateinit var lockoutRepository: LockoutRepository
     ...
     * [com.locke.app.ui.home.HomeViewModel.refreshDataDrivenHabits]) --
    ...
    private sealed class LockState {
        ...
        data class Locked(
            ...
            /**
             * True while a user-started [LockoutRepository] lockout is running --
             * checked right after bedtime and before everything else (habit gating, a
             * penalty, a grace token), same hard-commitment strength as bedtime, since
             * a self-imposed lockout that a grace token or a finished habit list can
             * talk you out of isn't much of a lockout. See
             * [com.locke.app.ui.block.LockoutScreen].
             */
             ...
             * [com.locke.app.ui.block.BlockOverlayViewModel] needs this
    ...
    private suspend fun evaluateLockState(): LockState {
        ...
        if (lockoutRepository.isLockoutActiveNow()) return LockState.Locked(isBedtime = false, isLockout = true)
    ...
    private fun showAppBlockScreen(
        ...
        val intent = Intent(this, BlockOverlayActivity::class.java).apply {
            ...
            putExtra(BlockOverlayActivity.EXTRA_IS_LOCKOUT, isLockout)
