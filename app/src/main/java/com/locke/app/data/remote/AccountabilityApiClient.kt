 ...
 *
 * Adding a buddy requires an active subscription -- [addBuddy] is the one call the
 * backend actually gates server-side on that (HTTP 402, checked on both sides of the
 * pairing since either device could be the one without premium); callers should check
...
interface AccountabilityApiClient {
    ...
    /** Redeems a buddy's pairing code with the backend, returning the newly paired buddy. 402 if either side lacks an active subscription. */
