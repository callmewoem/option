...
/** A broken-streak date paired with why it broke -- see [com.locke.app.data.local.entity.StreakScarEntity]. */
...
class HabitRepository @Inject constructor(
     ...
     * [com.locke.app.service.HealthConnectSyncWorker].
     */
     ...
     * heatmap. A date marked as a [com.locke.app.data.local.entity.StreakScarEntity]
    ...
    /** Dates in range marked as a broken streak (see [com.locke.app.data.local.entity.StreakScarEntity]) -- for the stats screen's "broken streaks" count. */
    ...
    /** How often each [com.locke.app.data.local.entity.StreakScarEntity.reason] shows up in range, most frequent first -- a "why did streaks break" summary. */
    ...
    /** See [ConsistencyStats]. Fetches [getDayScoresInRange] itself -- if a caller already has that map (e.g. [HabitsViewModel][com.locke.app.ui.habits.HabitsViewModel] building the same window's heatmap), call [consistencyStatsForDayScores] directly instead to avoid querying it twice. */
    ...
    /** Same dates as [getScarredDatesInRange], paired with why each streak broke -- for [com.locke.app.util.StatsExportUtil]'s data export. */
    ...
    /** `1 shl (dayOfWeek.value - 1)` for [date]'s day of week -- see [HabitEntity.scheduledDaysMask][com.locke.app.data.local.entity.HabitEntity]. */
    private fun dayBitFor(date: String): Int = 1 shl (DateProvider.fromDateString(date).dayOfWeek.value - 1)

    companion object {
        /** Habit types that [com.locke.app.service.HealthConnectSyncWorker] can sync. */
