...
import com.habitsfirst.androidclone.data.remote.BackendApiException
import com.habitsfirst.androidclone.data.remote.BackendConfig
import com.habitsfirst.androidclone.data.remote.DeviceIdentityRepository
 ...
 *
 * Photo verification is a premium feature: [entitlementRepository] is checked before
 * the request is even sent (cheaper than a round trip, and the backend enforces the
 * same rule server-side, HTTP 402, for defense in depth against a modified client).
 */
@Singleton
class BackendImageVerificationClient @Inject constructor(
    ...
    override suspend fun verify(request: VerificationRequest): VerificationResult =
        withContext(Dispatchers.IO) {
            if (!entitlementRepository.isPremium()) throw ImageVerificationException.RequiresPremium
