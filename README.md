...
| Camera | Required for *photo verification* and a *Scan a tag* habit's QR option -- capture is camera-only, with no gallery-picker fallback, so an old/swapped photo can't stand in for today's proof. |
| Internet | Talks to Locke's own backend (`backend/`) -- photo verification, accountability buddies, Google Play purchase verification, A/B experiment assignment, and analytics -- and syncs *GitHub*/*Code (WakaTime)* habits against their own APIs, only if you've set the relevant token/key in Settings → *Connections*. |
