 ...
 *
 * Requires an active (trial or paid) subscription -- enforced here server-side (in
 * addition to whatever the client checks) since every call costs real money against
 * ANTHROPIC_API_KEY, so a modified or older client can't grant itself checks it
 * shouldn't have by skipping its own local check.
 */
router.post('/verify-photo', requireDevice, async (req, res) => {
  const entitlement = getEntitlement(req.deviceId);
  if (!entitlement.isPremium) {
    return res.status(402).json({
      error: 'AI photo checks require a Locke Premium subscription -- start your free trial.',
    });
  }
